...
if platform.system() == "Darwin":
    os.environ["QT_QPA_PLATFORM"] = "cocoa"  # macOS-only GUI fix; do NOT set on Windows
...
def get_participant_id():
    """Prompt once for a participant ID via a GUI dialog; used by both sections."""
    dlg = gui.Dlg(title="Participant Info")
    dlg.addField("Participant ID:")
    ok_data = dlg.show()
    if not dlg.OK or not ok_data or not ok_data[0].strip():
        core.quit()
    participant_id = ok_data[0].strip()
...
if not dlg.OK:
    core.quit()
session_value = (ok_data[0].strip() if ok_data and ok_data[0].strip() else "1")
participant_data = {
